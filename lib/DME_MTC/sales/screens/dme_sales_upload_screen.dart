...
import '../services/dme_sale_service.dart';
import '../../customers/services/dme_customer_service.dart';
...
// ── Preview item status ───────────────────────────────────────────
enum _RecordStatus { pending, checking, matchFound, newCustomer, conflict, duplicateSale }
...
class _DmeSalesUploadPageState extends State<DmeSalesUploadPage> {
  ...
  final _customerSvc = DmeCustomerService.instance;
  final _saleSvc = DmeSaleService.instance;
  final _userSvc = DmeUserService.instance;
  final _reminderSvc = DmeReminderService.instance;
  ...
  Future<void> _updateReminderAsync({
    ...
    try {
      ...
      await _retryWithBackoff(
        operation: () => DmeReminderService.instance.scheduleReminder(
                        ...
                        children: [
                          ...
                          if (item.hasCategoryConflict) ...[
                            ...
                            RadioListTile<bool>(
                              ...
                              title: Text(
                                'Keep existing: ${item.existingCategory ?? "—"}',
                          ...
                          if (item.hasTypeConflict) ...[
                            ...
                            RadioListTile<bool>(
                              ...
                              contentPadding: EdgeInsets.zero,
                              title: Text(
                                'Keep existing: ${item.existingType ?? "—"}',
                                style: const TextStyle(fontSize: 12),
                              ),
                              value: false,
  ...
  Future<void> _checkRecords() async {
    ...
    for (final item in _items!) {
      ...
      try {
        ...
        final existing = await _customerSvc.findCustomerByPhone(phone);
        if (mounted && _items != null) {
          if (existing != null) {
            ...
            String? dbCat;
            String? dbType;
            final custBranches = await _customerSvc.getCustomerBranches(existing.id!);
            if (custBranches.isNotEmpty) {
              final firstB = custBranches.first;
              if (firstB['dme_categories'] is Map) {
                dbCat = (firstB['dme_categories'] as Map)['name'] as String?;
              }
              if (firstB['dme_customer_types'] is Map) {
                dbType = (firstB['dme_customer_types'] as Map)['name'] as String?;
              }
            }

            // Check if sale already exists for this customer on the exact same date
            final existingSaleItems = await _saleSvc.getSaleItemsByCustomerDate(existing.id!, item.record.date);
            final isDuplicateSale = existingSaleItems.isNotEmpty;

            // Detect category / type mismatches
            final excelCat = item.record.category?.trim().toUpperCase();
            final dbCatUpper = dbCat?.trim().toUpperCase();
            final catConflict = excelCat != null &&
                excelCat.isNotEmpty &&
                dbCatUpper != null &&
                dbCatUpper.isNotEmpty &&
                excelCat != dbCatUpper;

            final excelType = item.record.customerType?.trim().toUpperCase();
            final dbTypeUpper = dbType?.trim().toUpperCase();
            final typeConflict = excelType != null &&
                excelType.isNotEmpty &&
                dbTypeUpper != null &&
                dbTypeUpper.isNotEmpty &&
                excelType != dbTypeUpper;

            setState(() {
              item.existingCustomer = existing;
              item.existingCategory = dbCat;
              item.existingType = dbType;
              item.isDuplicateSale = isDuplicateSale;
              item.status = isDuplicateSale
                  ? _RecordStatus.duplicateSale
  ...
  /// Updates category / type on an existing customer branch mapping if the user
  ...
  Future<void> _applyExistingCustomerCategoryTypeUpdate({
    ...
    final newCategory = updateCat ? effectiveCategory : item.existingCategory;
    final newType = updateType ? effectiveType : item.existingType;
    ...
    await _retryWithBackoff(
      operation: () => _customerSvc.insertCustomerBranch(
        customerId: customerId,
        branchId: branchId,
  ...
  Future<void> _upload() async {
    ...
    final dmeUser = uid != null ? await _userSvc.getCurrentUser(uid) : null;
    ...
    for (final item in _items!) {
      ...
      // Skip duplicate sale upload if sale for this customer & date already exists in DB
      if (item.status == _RecordStatus.duplicateSale || item.isDuplicateSale) {
        ignored++;
        continue;
      }
      ...
      try {
        ...
        if (item.status == _RecordStatus.matchFound) {
          ...
          await _retryWithBackoff(
            operation: () =>
                _customerSvc.updateLastPurchaseDate(customerId!, record.date),
          );
          ...
          // Phone matches, name differs → keep existing
          customerId = item.existingCustomer!.id;
          await _retryWithBackoff(
            operation: () =>
                _customerSvc.updateLastPurchaseDate(customerId!, record.date),
          ...
          final newCust = await _retryWithBackoff(
            operation: () => _customerSvc.upsertCustomer(DmeCustomer(
        ...
        await _retryWithBackoff(
          operation: () => _saleSvc.insertSale(sale),
        );
        ...
        content: Column(
          ...
          children: [
            if (success > 0)
              ...
              const Text('⚠️ No new records were uploaded (All records already exist in database)',
  ...
  Widget _buildPreview() {
    ...
    final duplicateCount =
        items.where((i) => i.status == _RecordStatus.duplicateSale).length;
              ...
              if (_checking) ...[
                ...
                Wrap(
                  ...
                  children: [
                    ...
                    if (duplicateCount > 0)
                      _chip('$duplicateCount duplicate date', Colors.purple),
                  ],
                ),
              ],
            ],
      ...
      child: Material(
        ...
        subtitle: Column(
          ...
          children: [
            ...
            if (!shouldCreateReminderForCategory(item.resolvedCategory ?? r.category)) ...[
              const SizedBox(height: 2),
              Row(
                children: const [
                  Icon(Icons.notifications_off_outlined, size: 12, color: Colors.blueGrey),
                  SizedBox(width: 4),
                  Text(
                    'No reminder (Excluded category)',
                    style: TextStyle(
                      fontSize: 11,
                      fontWeight: FontWeight.w600,
        ...
        children: [
          // ── Duplicate sale warning notice ────────────────────
          if (item.isDuplicateSale) ...[
            Container(
              width: double.infinity,
              ...
              child: Row(
                children: [
                  Icon(Icons.copy_rounded, size: 18, color: Colors.purple.shade700),
                  const SizedBox(width: 8),
                  Expanded(
                    child: Text(
                      'A sale record for ${item.existingCustomer?.name ?? "this customer"} on ${r.date.toString().split(' ')[0]} already exists in the database.',
                      style: TextStyle(
                        fontSize: 12,
          ...
          // ── No reminder notice if category is excluded ────────────────────
          if (!shouldCreateReminderForCategory(item.resolvedCategory ?? r.category)) ...[
            Container(
              width: double.infinity,
              padding: const EdgeInsets.all(10),
              ...
              child: Row(
                children: [
                  Icon(Icons.notifications_off, size: 18, color: Colors.blueGrey.shade700),
                  const SizedBox(width: 8),
                  Expanded(
                    child: Text(
                      'Category "${item.resolvedCategory ?? r.category ?? '—'}" is excluded from automatic reminders.',
                      style: TextStyle(
                        fontSize: 12,
                        fontWeight: FontWeight.w500,
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (item.hasCategoryConflict)
                    Text(
                      'Category — DB: ${item.existingCategory}  |  Excel: ${item.record.category}',
                      style: const TextStyle(fontSize: 12),
                    ),
                  if (item.hasTypeConflict) ...[
                    if (item.hasCategoryConflict) const SizedBox(height: 4),
                    Text(
                      'Type — DB: ${item.existingType}  |  Excel: ${item.record.customerType}',
                      style: const TextStyle(fontSize: 12),
                    ),
                  ],
                  const SizedBox(height: 8),
